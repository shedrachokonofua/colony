...
- **Agent identity:** agents use role-keyed provider bot accounts (`engine`, `reviewer`, `architect`, `integrator`, `memory_consolidator`, `supervisor` by default) with scoped tokens. The Tool Gateway resolves the calling role to the matching bot token and enforces capability + namespace checks before provider calls. Bot users, PATs, OAuth app, and webhook are created by a `ProviderAdapter.bootstrap()` operation that takes one admin PAT.
