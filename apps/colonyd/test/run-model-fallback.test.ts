...
describe("run event sink persists pi_model_fallback", () => {
  ...
  it("persists reviewer finalizer and fallback events", async () => {
          ...
          response.write(
            `data: ${JSON.stringify({
              ...
              choices: [
                {
                  index: 0,
                  delta: { role: "assistant", content: "Reviewing diff." },
                  finish_reason: null,
                },
              ],
            })}\n\n`,
    ...
    const dbDir = mkdtempSync(join(tmpdir(), "colony-finalizer-store-"));
    scratchDirs.push(dbDir);
    const store = new Store(join(dbDir, "test.db"));
    const scope = store.createScope({
      goal: "finalizer sink",
      title: "finalizer sink",
      provider_repo_id: "1",
      provider_repo_path: "so/fake",
    });
    const run = store.startRun({
    ...
    const scratchDir = mkdtempSync(
      join(tmpdir(), "colony-finalizer-colonyd-test-"),
    );
    scratchDirs.push(scratchDir);
    const runner = new PiBaseAgentRunner(
      {
    ...
    const events = store.listRunEvents(run.id).events;
    const finalizerForced = events.find(
      (e) => e.event === "pi_finalizer_forced",
    );
    expect(finalizerForced).toBeTruthy();
    expect(JSON.parse(finalizerForced!.detail_json)).toMatchObject({
      model: "primary",
    });

    const fallback = events.find((e) => e.event === "pi_model_fallback");
    expect(fallback).toBeTruthy();
    expect(JSON.parse(fallback!.detail_json)).toMatchObject({
      from: "primary",
      to: "fallback",
