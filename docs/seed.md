...
- **Agent identity:** agents share two provider bot accounts (`colony-engine` and `colony-reviewer`) with scoped tokens; the split is forced by GitLab's MR self-approval rule, and per-role attribution lives in Colony's audit log. Bot users, PATs, OAuth app, and webhook are created by a `ProviderAdapter.bootstrap()` operation that takes one admin PAT.
